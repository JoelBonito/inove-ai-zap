...
5. 19:30 — 20:00 (00:30)
   - Atividades:
     - Upgrade no `useAuth` para buscar roles e permissões (Admin) do Firestore.
     - Implementação completa da página de Configurações (`Settings.tsx`):
       - Gestão de API Keys (UAZAPI) com persistência em `config/uazapi`.
       - Alteração de senha com reautenticação de segurança.
       - Persistência de preferências de notificação.
     - Refinamento de tradução de erros de campanha (tratamento de `rate-limit`).
